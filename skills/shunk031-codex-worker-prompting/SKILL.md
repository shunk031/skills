...
6. **Write the dispatch in the artifact's target register.** A worker mirrors dispatch vocabulary and constraints into deliverables. For reader-facing artifacts, keep internal codenames and audit constraints out of the prompt body, and state that compliance evidence belongs in the PR body, not the document.
   For a task whose deliverable is a reader-facing report, require the worker to read the shunk031-writing-kosshi skill at `~/.agents/skills/shunk031-writing-kosshi/SKILL.md` first, then read the shunk031-writing-telegraph skill at `~/.agents/skills/shunk031-writing-telegraph/SKILL.md`, and apply both to the report body. Keep this requirement scoped to the report, not the dispatch or routine progress/final replies unless the user asks for that style.
