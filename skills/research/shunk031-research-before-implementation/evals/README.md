...
The skill treats this provider restriction as stable: it does not retry the denied provider or change credentials, and uses another permitted retrieval path only when the host exposes one. Transient gateway failures receive a short pause and up to three attempts. After the permitted paths fail, the worker reports the exact research question as `BLOCKED`. The evaluation needs live retrieval to measure that behavior, so the marker is the explicit harness boundary.
