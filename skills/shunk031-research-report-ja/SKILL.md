...
- Make the header or title state the tested question in plain words; never use internal codenames or process labels such as chain indices（段 1、rung N）or execution labels（model-free、CPU、offline）as reader-facing identifiers or header content. Reason: internal labels make a report depend on context the reader does not have.
