...
- Read receipts: Immediately after the frontmatter and before the first heading, every `SKILL.md` opens with a NOTE block that says `<emoji> 私は <skill-name> を読みました。 / I read <skill-name>.` on one line, with Japanese and English aligned; the emoji fits the skill's subject, `<skill-name>` matches the frontmatter name, and the receipt makes skill loading observable in the transcript.
