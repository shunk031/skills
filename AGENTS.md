...
- Completion-only cases: An empty `skills/<name>/evals/completion-mode` marker opts that skill's behavior and trigger gates into Shuhari completion mode. Use it only when the cases need neither tools nor an agent workspace: completion measures the prompted response or invocation decision, not filesystem consultation or command execution. It cannot coexist with `evals/network-required` or `evals/tools-required`; agentic remains the default. `SHUHARI_MODE` is an explicit development override and wins over the marker.
